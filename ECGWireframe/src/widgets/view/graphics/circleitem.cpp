...
void CircleItem::addPoint(const QVector4D &point) {
	m_points.push_back(point);
}

void CircleItem::setScale(qreal scale) {
	m_scale = scale;
...
void CircleItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget) {
	...
	qsizetype n = m_points.count();

	qreal pw, px, py;
	pw = m_matrixPtr[12] * m_points[n - 1].x() + \
		m_matrixPtr[13] * m_points[n - 1].y() + \
		m_matrixPtr[14] * m_points[n - 1].z() + \
		m_matrixPtr[15] * m_points[n - 1].w();
	px = m_matrixPtr[0] * m_points[n - 1].x() + \
		m_matrixPtr[1] * m_points[n - 1].y() + \
		m_matrixPtr[2] * m_points[n - 1].z() + \
		m_matrixPtr[3] * m_points[n - 1].w();
	py = m_matrixPtr[4] * m_points[n - 1].x() + \
		m_matrixPtr[5] * m_points[n - 1].y() + \
		m_matrixPtr[6] * m_points[n - 1].z() + \
		m_matrixPtr[7] * m_points[n - 1].w();
	if (pw != 0) {
		px /= pw;
		py /= pw;
	}
